...
# ===================================================
# ⭐️ 1-2. 히스토리 변환 및 30턴 제한 함수 (새로 추가됨)
# ===================================================
...
def format_and_truncate_history(messages, max_turns=30):
    ...
    """
    # 마지막 max_turns (30)개만 선택하여 Context Window 초과를 방지합니다.
    ...
    for message in history_to_send:
        ...
        # Gemini API는 챗봇 역할을 'model'로, 사용자 역할을 'user'로 기대합니다.
        gemini_role = "model" if role == "assistant" else "user"
        
        # ⭐️ 중요: API에 전달할 때는 Streamlit 출력용 마크다운 포맷(**[이름]:**)을 제거해야 합니다.
        # **[이름]: 대화** 형식에서 이름 부분을 제거하는 패턴 (user/assistant 공통)
        clean_text = re.sub(r'\*\*\[[^\]]+\]\*\*[:\s]*', '', content_text, 1).strip()
            
        if clean_text:
             # Parts는 API에 전달되는 텍스트 내용입니다.
...
# ⭐️ 3. 모델 초기화 함수
# 
# 🚨 Chat 객체 대신 GenerativeModel 객체만 반환하도록 변경했습니다.
# ===================================================
...
def initialize_model(): 
    ...
    # 시스템 프롬프트는 initialize_model에서 구성하여 반환된 model 객체에 설정됩니다.
    system_prompt = f"""
...
# chat 대신 model 객체를 저장하며, 초기값은 None입니다.
...
if st.session_state.model is None or st.sidebar.button("새 채팅 시작", key="restart_chat_btn"): 
    if user_role_input:
        ...
        # initialize_model이 user_role에 의존하므로, user_role을 인수로 전달합니다.
