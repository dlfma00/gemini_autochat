...
def initialize_model(user_role, session_id): # 세션 분리 위해 session_id 인자 사용
    ...
    system_prompt = f"""
    [규칙]: 당신은 아래 6명의 캐릭터를 동시에 연기합니다. 사용자 역할에 맞게 자연스럽게 2~4명이 대화에 참여하세요. 출력 형식은 반드시 "[이름]: 대사"로만 작성합니다. (지문 금지, 구어체 사용). 말투: 트위터, 카톡, SNS 하듯 적어주세요. 문장부호를 반드시 지키지 않고, 급하면 오타가 날 수도 있습니다.
